...
function addTag(wrapper, tagText, tagsArray, inputElem) {
  ...
  var productVariantTypeColor = [{"value": "red", "text": "Red"}, {"value": "blue", "text": "Blue"}, {"value": "gray", "text": "gray"}];
  var productVariantTypeSize = [{"value": "small", "text": "Small"}, {"value": "large", "text": "Large"}];

function AddVariant() {
var $CustomvariantTypes = $("#CustomVariant_Type");
var $VariantType = $CustomvariantTypes.val();
var $VariantTypeTitle = $("#CustomVariant_Type option:selected").text();
if ($VariantType == 0 || $VariantType == null || $VariantType == "") {
  Swal.fire({
      icon: "error",
...
$("#SaveVariantButton").remove();
var NewCategory = '';

// For predefined 'Color' options
if ($VariantType == 1) {
  NewCategory = `
      <div class='VariantDiv row'>
          <div class="col-md-5">
              <label>Variant Type</label>
              <input type="type" class="form-control mainvariant" disabled data-id="${$VariantType}" value="${$VariantTypeTitle}" />
          </div>
          <div class="col-md-6">
              <label>Variant ${$VariantTypeTitle}</label>
              <select class="form-control mainvariantValue select2" multiple="multiple" style="width: 100%;"></select>
          </div>
          <div class="col-md-1">
              <button type="button" class="btn btn-danger remove-variant-row"><i class="fa fa-trash"></i></button>
          </div>
      </div>`;
  $("#VariantTypeMainRow").append(NewCategory);
  productVariantTypeColor.forEach(function(item) {
      $(".select2").append(`<option value="${item.value}">${item.text}</option>`);
  });
}
// For predefined 'Size' options
  ...
  $("#VariantTypeMainRow").append(NewCategory);
  productVariantTypeSize.forEach(function(item) {
      $(".select2").append(`<option value="${item.value}">${item.text}</option>`);
  });
}
// For 'Custom' variant type
  ...
  NewCategory = `
      <div class='VariantDiv row'>
          ...
          <div class="col-md-6">
              ...
              <input type="text" class="form-control mainvariantValue" data-role="tagsinput" placeholder="Add custom values" />
          </div>
          <div class="col-md-1">
              <button type="button" class="btn btn-danger remove-variant-row"><i class="fa fa-trash"></i></button>
          </div>
...
$("#VariantTypeMainRow").append(`<div class="form-actions mt-10 text-end" id="SaveVariantButton"><button type="button" class="btn btn-warning" onclick="generateTable()">Save Variant</button></div>`);
$('.select2').select2();
}
  ...
  function generateTable() {
      var customsArrays = [];  
      $("#VariantTypeMainRow .VariantDiv").each(function () {
          var CustomCollection = {};
          var customs = [];
CustomCollection.Key = $(this).find(".mainvariant").val().trim();
var isTagInput = $(this).find(".bootstrap-tagsinput").length > 0;
if (isTagInput) {
$(this).find(".bootstrap-tagsinput .tag").each(function () {
customs.push($(this).text().trim());});
} else {
$(this).find(".mainvariantValue option:selected").each(function () {
  customs.push($(this).text().trim());
              });
          }
          CustomCollection.Value = customs;
          customsArrays.push(CustomCollection);
      });

      $("#VariantTypeRowTable thead").empty();
      $("#VariantTypeRowTable tbody").empty();

      let headerRow = "<tr>";
      $(".mainvariant").each(function () {
          headerRow += `<th>${$(this).val().trim()}</th>`;
      });
      headerRow += "<th>Price</th><th>Stock</th></tr>";
      $("#VariantTypeRowTable thead").append(headerRow);

      const combinations = generateCombinations(customsArrays.map(obj => obj.Value));
      combinations.forEach((combination, index) => {
          let row = "<tr>";
          combination.forEach(option => {
              row += `<td>${option}</td>`;
          });
          row += `<td><input type="number" class="form-control" placeholder="Price"></td><td><input type="number" class="form-control" placeholder="Stock"></td></tr>`;
          $("#VariantTypeRowTable tbody").append(row);
      });
  }

  function generateCombinations(variantArrays) {
      if (variantArrays.length === 0) return [[]];
      const firstArray = variantArrays[0];
      const restCombinations = generateCombinations(variantArrays.slice(1));
  ...
  $(document).on('click', '.remove-variant-row', function () {
      $(this).closest('.VariantDiv').remove();
  });
...
// function generateTable() {
//   var customsArrays = [];  
//   $("#variantList .variantRow").each(function () {
//       var CustomCollection = {};
//       var customs = [];
//       CustomCollection.Key = $(this).find(".mainvariant").val().trim();
//       var isTagInput = $(this).find(".bootstrap-tagsinput").length > 0;
...
//           row += `<td>${option}</td>`;
//       });
//       row += `<td><input type="number" class="form-control" placeholder="Price"></td>
//       <td><input type="number" class="form-control" placeholder="Stock"></td>
//       </tr>`;
//       $("#VariantTypeRowTable tbody").append(row);
//   });
// }
